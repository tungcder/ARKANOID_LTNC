...
public class BrickMove extends Brick implements Movable {
    ...
    public void initMovementRange(BrickGrid brickGrid) {
        ...
        for (Brick b : allBricks) {
            if (!b.isDestroyed() && Math.abs((b.getY() + b.getHeight() / 2.0) - centerY) < tolerance && b.getX() + b.getWidth() <= x) {
        ...
        for (Brick b : allBricks) {
            if (!b.isDestroyed() && Math.abs((b.getY() + b.getHeight() / 2.0) - centerY) < tolerance && b.getX() >= x + width) {
